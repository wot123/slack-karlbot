...
%%--------------------------------------------------------------------
init([File]) ->
    State = luerl:load_module([karl], luerl_lib_karl, luerl:init()),
    {_, State2} = luerl:dofile(File, State),
    lager:info("lua_wrapper init"),
    syn:join(slack_messages, self()),
    {ok, #state{lua_state = State2}}.
